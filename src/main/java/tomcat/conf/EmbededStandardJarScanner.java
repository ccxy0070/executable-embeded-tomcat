 ...
 *
 * <pre>
 * When boot by spring boot loader, WebappClassLoader.getParent() is LaunchedURLClassLoader, Just need to scan WebappClassLoader and LaunchedURLClassLoader.
 *
 * When boot in IDE, WebappClassLoader.getParent() is AppClassLoader, Just need to scan WebappClassLoader and AppClassLoader.
 * </pre>
 *
 * @see {@link JarScanner}
 *
...
public class EmbededStandardJarScanner implements JarScanner {
	...
	private void process(JarScanType scanType, JarScannerCallback callback, URL url, String webappPath,
		...
		if ("jar".equals(url.getProtocol()) || url.toString().endsWith(".jar")) {
		    try (Jar jar = JarFactory.newInstance(url)) {
		    	if (isScanManifest()) {
	                processManifest(jar, isWebapp, null);
	...
	private void process2(JarScanType scanType, JarScannerCallback callback, URL url, String webappPath,
			boolean isWebapp) throws IOException {
